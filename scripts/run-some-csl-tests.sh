    ...
    if [ $(stat -f "%z" "${csl}") -gt 2000 ]; then
        ...
        echo "cat  ${my_markdown}                                               \
              ...
              | pandoc ${my_pandoc_options}                                     \
                --csl ${csl}                                                    \
                -o ${latex_engine}---pandoc-csl-testing---${cslname/.csl/.pdf}  \
                -
             "
        ...
        if [ "x${return_value}" != "x0" ]; then
            echo -n " $(date)  :   ERROR processing source Markdown file ${my_markdown} with ${latex_engine} + style file ${cslname}" \
                  | tee -a ${latex_engine}---git.pandoc-csl-testing.log
            echo  | tee -a ${latex_engine}---git.pandoc-csl-testing.log
        fi
        ...
        echo "cat  ${my_markdown}                                       \
              ...
              | pandoc ${my_pandoc_options}                             \
                --csl ${csl}                                            \
                --standalone                                            \
                -o html---pandoc-csl-testing---${cslname/.csl/.html}    \
                -
             "
        ...
        if [ "x${return_value}" != "x0" ]; then
            echo -n " $(date)  :   ERROR converting source Markdown file ${my_markdown} + style file ${cslname} to HTML" \
                  | tee -a html---git.pandoc-csl-testing.log
            echo  | tee -a html---git.pandoc-csl-testing.log
        fi
        ...
        echo "cat  ${my_markdown}                                   \
              ...
              | pandoc ${my_pandoc_options}                         \
                ...
                --standalone                                        \
                --to=epub3                                          \
                -o epub3-pandoc-csl-testing---${cslname/.csl/.epub} \
                -
             "
        ...
        if [ "x${return_value}" != "x0" ]; then
            echo -n " $(date)  :   ERROR converting source Markdown file ${my_markdown} + style file ${cslname} to EPUB3" \
                  | tee -a epub3---git.pandoc-csl-testing.log
            echo  | tee -a epub3---git.pandoc-csl-testing.log
        fi
...
echo "ERRORS for EPUB3 output:"
    grep ERROR epub---git.pandoc-csl-testing.log
echo

echo "ERRORS for HTML output:"
    grep ERROR html---git.pandoc-csl-testing.log
echo

echo "ERRORS for PDF output:"
    grep ERROR *latex---git.pandoc-csl-testing.log
echo

## Now look at the actual files generated by above commands;
## Linux users have to use `xdg-open` instead of `open`:
open epub3-pandoc-csl-testing---*.epub
open html-pandoc-csl-testing---*.html
open *latex-pandoc-csl-testing---*.pdf
# xdg-open epub3-pandoc-csl-testing---*.epub
# xdg-open html-pandoc-csl-testing---*.html
# xdg-open *latex-pandoc-csl-testing---*.pdf
