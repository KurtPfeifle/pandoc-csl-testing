...
mkdir ${my_output} ${my_logdir} 2>/dev/null
...
my_localizations="-V lang=en-US -V otherlangs=de-DE"
#my_localizations="-V lang=de-DE -V otherlangs=en-US"
...
# The following is required because `stat` has different syntax on Mac and on Linux.
# Also, `open` does not work on Linux, it needs `xdg-open`.
case "$(uname)" in
    Darwin)
        my_stat_command="stat -f %z "
        my_open=open
        ;;
    Linux)
        my_stat_command="stat -c %s "
        my_open=xdg-open
        ;;
esac

time for csl in ~/svn-git-stuff/git.csl-styles/{stuttgart-media-university,small}*.csl; do
...
#time for csl in ~/svn-git-stuff/git.csl-styles/{thomson-reuters-legal-tax,triangle,universidade-de-sao-paulo-instituto,wissenschaftlicher-industrielogistik-dialog,stuttgart-media-university,thieme-e-journals-vancover,rapid-communications-in-mass-spectrometry,rtf-scan,small,multiple-sclerosis-journal,norma-portuguesa-405,lluelles-lexisnexis}*.csl; do
    ...
    #if [ $(${my_stat_command} "${csl}") -gt 2000 ]; then
    if [ $(${my_stat_command} "${csl}") -gt 2000 ]; then
        ######## LaTeX/PDF... ##########################################################
        ...
        if [ "x${return_value}" != "x0" ]; then
            ...
            #echo "${csl} is not a decent CSL style file: filesize is only $(${my_stat_command} "${csl}") Bytes." \
            echo "${csl} is not a decent CSL style file: filesize is only $(stat -c "%s" "${csl}") Bytes." \
                  | tee -a ${my_logdir}/epub3---git.pandoc-csl-testing.log
            echo  | tee -a ${my_logdir}/epub3---git.pandoc-csl-testing.log
    fi
...
#for i in ${my_output}/epub3---pandoc-csl-testing---*.epub; do ${my_open} "${i}"; done
#for i in ${my_output}/html---pandoc-csl-testing---*.html;  do ${my_open} "${i}"; done
for i in ${my_output}/*latex---pandoc-csl-testing---*.pdf;  do ${my_open} "${i}"; done
